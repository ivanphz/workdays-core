...
import { exceptionClosed } from './exceptions.js';
...
export function createUsMarketProvider() {
  ...
  return {
    ...
    isOffDay(dateStr) {
      return records.has(dateStr) || exceptionClosed(dateStr, 'market') != null;
    },

    fact(dateStr) {
      const r = records.get(dateStr);
      if (r) return { isHoliday: true, isMakeup: false, names: r.name != null ? { en: r.name } : {}, observed: r.observed, nominalDate: r.nominalDate };
      const ex = exceptionClosed(dateStr, 'market');
      if (ex) return { isHoliday: true, isMakeup: false, names: ex.name != null ? { en: ex.name } : {}, observed: false, nominalDate: null };
      return null;
    },
