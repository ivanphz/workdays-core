...
export function createArchiveProvider({ dataset, tag, sourceName, legacyLang, data, sliceDays, fetchLive }) {
  ...
  return {
    ...
    async load(years, ctx = {}) {
      const rows = [], logs = [];
      let live = null;
      if (ctx.dataSource === 'online' && fetchLive) {
        const raw = await fetchLive(ctx.fetchImpl);
        if (raw) {
          live = {};
          for (const [d, e] of Object.entries(raw)) live[d] = normalizeDayEntry(e, legacyLang);
        }
        logs.push(live ? `[${tag}] online:抓取成功(${Object.keys(live).length} 天)` : `[${tag}] online:抓取失败,退用归档`);
      }
