...
`alarm-api`(iOS 闹钟网关)与 `calendar-api`(信用卡/签到提醒)各自维护了一套
...
这与 calendar-api 原实现的**日期集合逐日等价**(测试钉死),增量只是元数据。
...
`official` 保留为**永久别名**(calendar-api 的 `?cnRule=official` 与配置里的
...
## 3. 兼容承诺(对 calendar-api 的逐语义等价清单)
...
calendar-api 现有调用天然满足;alarm-api 迁移配方里 years 已含昨/今/明三日的年份。
...
第三条同时拆掉了 v1 的 E3 兼容钉子 —— 实践中该路径从未触发(calendar-api 建 hub 时取全量 token 并集),拆除零风险。
...
# v3.0.0 迭代记录 — 数据集插件化(输入输出两分 + 故障隔离)

> 触发:Ivan 连环追问——文件名为何两位不三位?为何一个 refresh 脚本管所有国?加新国要改中心脚本=不解耦;能否给 AI 一份文档就生成"数据集+更新脚本+历史数据",且 AI 失败/数据坏不影响已有国?这些全部指向同一病灶:v2.3 把"数据集"散成了数据(data/)+读取(providers/)+抓取(sources.js 段)+编排(refresh 块)+两处注册,共六处、三个中心文件。

## 13.1 架构:一国 = 一个自包含文件夹(插件化)

`src/datasets/<code>/`(index 清单 + data + provider + fetch + translations + 可选 exceptions),
中心只剩 `_loader.js`(发现+容错装载)。`tokens.js`/`i18n.js` 的国家表、译名表**全部从清单动态构建**
(buildTokenTables/buildTranslations),`index.js` 装载后注入。**加新国 = 放文件夹 + REGISTRY 加一行**,
中心代码零改动。彻底解决"改 refresh 脚本"的耦合。

## 13.2 故障隔离(Ivan 的硬要求:AI 失败不拖累已有国)

REGISTRY 用懒加载 `() => import()`,loader 逐个 try/catch,清单校验(code 两位、regions 非空、
createProviders 存在)不过即**判定坏数据集,跳过+告警,其余照跑**。已写测试:故意造语法错数据集,
已有国家全绿、loadLogs 出现 `[dataset:xx] 装载失败`。refresh 脚本同款容错(某国 fetch 抛错→保留归档、
继续其余)。为何用显式 REGISTRY 而非扫目录:Workers/bundler 无法运行时列目录、也需静态 import 打包。

## 13.3 输入输出两分(Ivan 提出的框架总纲)

写进 DATASET-GUIDE.md 开篇。输入侧四类源(官方多语/算法/第三方加工品/爬虫)+ 人工例外,允许无限
花样;输出侧唯一窄腰(provider 契约 → hub 统一输出),精确稳定。归一点 = fetch.js。**换源**:fetch.js
内部分"取(拉字节,多源多格式降级)+译(私有格式→schema 2)"两层,换源只动这一个文件,爆炸半径锁死,
provider/data/别国不动——这是"广泛兼容 + 精确稳定"的工程兑现。

## 13.4 US 特殊假期 = 算法型的人工例外层(回答"接口要我自己找吗")

答:不用找,也无权威机读源可找(白宫不发 JSON,是临时行政令)。总统哀悼日、NYSE 因灾休市归入
`us/exceptions.js`(算法覆盖层,标准可选件):`{date, scope:'bank'|'market'|'both', isOpen:false}`,
罕见事件手工加一行发 patch(频率约几年一次)。provider 的 isOffDay/fact 叠加例外查询。关键区分仍在:
联储清算(bank 口径)在哀悼日照常→算法精确;NYSE(market)会休→例外层补。

## 13.5 命名规则成文(回答"为何两位不三位")

此前是隐性约定,v3 写进 DATASET-GUIDE §2:文件夹/code/dataset id 前缀=alpha-2 小写(内部 canonical,
与 fact.region、已部署配置一致);token 输入主写法=alpha-3(文档引导),双射由清单 alpha3 字段声明、
归一在 parseToken 单一入口。dataset id:单历=region、多历=region-变体(GB-EAW、US-NYSE)。

## 13.6 HK 三语历史归档

Ivan 提供 1823 官方三语历史快照(31 份 JSON,2019–2026 十份×三语)。合并出 2018–2027 十年、170 天、
**全部三语齐全**(sc/tc/en),写入 datasets/hk/data.js。1823 官方本有 JSON+ICS 两格式且都定期更新 →
fetch.js 每语言走 JSON→ICS 降级、三语全收、日期×语言只增不删。

## 13.7 迁移与文件变动

providers/ 与 src/data/ 与 sources.js **整体删除**,内容迁入各 datasets/<code>/;中心 translations.js
拆散进各国文件夹。新增 docs/DATASET-GUIDE.md(数据集开发规范,给 AI 生成用)。测试 34 用例全绿
(provider 行为测试改用通用引擎注入 fixture;parseToken 独立使用前需 initDatasets/建 hub)。
gb/sg data.js 为 schema 2 空种子(线上有真实数据,交付不覆盖——但 v3 是文件夹结构重构,
gb/sg 数据文件路径变了,**本次需整仓上传**,首刷后自动填充)。

## 13.8 版本

v3.0.0(major:文件结构大改、providers/sources 路径移除;对外 API 与判定结果不变,下游 import
仍是 `createHolidayHub` 等,但 core 内部重组)。下游 alarm-api/calendar-api **无感**(只 import 顶层)。
