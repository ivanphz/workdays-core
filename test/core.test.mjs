...
import { createHolidayHub, parseToken, exportJson, exportIcs } from '../src/index.js';
import { createHkProvider } from '../src/providers/hk.js';
import { createGbProvider } from '../src/providers/gb.js';
import { createSgProvider } from '../src/providers/sg.js';
import { parseHkIcs, parseHkJson } from '../src/sources.js';
...
test('D3 ICS 解析器: DTSTART+SUMMARY 抽取, RFC5545 折行反折叠', () => {
  const ics = [
    'BEGIN:VCALENDAR',
    'BEGIN:VEVENT', 'DTSTART;VALUE=DATE:20260101', 'SUMMARY:The first day of January', 'END:VEVENT',
    'BEGIN:VEVENT', 'DTSTART;VALUE=DATE:20260217', 'SUMMARY:Lunar New Year', " 's Day", 'END:VEVENT',
    'END:VCALENDAR'
  ].join('\r\n');
  const map = parseHkIcs(ics);
  assert.equal(map.get('2026-01-01'), 'The first day of January');
  assert.equal(map.get('2026-02-17'), "Lunar New Year's Day"); // 折行已并回
});

test('D4 1823 JSON 解析(主源形态,官方快照同款结构)', () => {
  const map = parseHkJson({ vcalendar: [{ prodid: 'x', vevent: [
    { dtstart: ['20210701', { value: 'DATE' }], summary: '香港特别行政区成立纪念日', uid: 'a' },
    { dtstart: ['20210922', { value: 'DATE' }], summary: '中秋节翌日', uid: 'b' },
    { dtstart: 'garbage', summary: '坏数据', uid: 'c' }
  ] }] });
  assert.equal(map.size, 2);
  assert.equal(map.get('2021-07-01'), '香港特别行政区成立纪念日');
});
...
test('E3 v2 响亮降级: 非法口径 → 告警+默认;数据集未加载 → 告警+纯周末(联邦退化钉子已拆)', async () => {
  ...
  assert.ok(hub.loadLogs.some(l => l.includes("未识别的口径 'CN:official'")));
...
test('J2 online 抓取失败 → 退用归档(mode=bundled),行为与默认完全一致', async () => {
  ...
  assert.ok(hub.loadLogs.some(l => l.includes('online') && l.includes('0/1')));
});
